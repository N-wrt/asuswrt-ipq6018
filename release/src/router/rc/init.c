...
//#define XP4_OLD
	case MODEL_PLAX56XP4:
		...
		nvram_set_int("led_blue_gpio", 79);
	    if (xp4_old) {
		nvram_set_int("led_green_gpio", 33);
		nvram_set_int("led_red_gpio", 32);
	    } else {
		nvram_set_int("led_green_gpio", 58);
		nvram_set_int("led_red_gpio", 57);
	    }
		nvram_set_int("btn_rst_gpio_1", 56|GPIO_ACTIVE_LOW);
		nvram_set_int("btn_wps_gpio_1", 72|GPIO_ACTIVE_LOW);
		nvram_set_int("pwr_usb_gpio", 22);
		...
		if (nvram_match("HwId", "B")) { // Node, follow CD6N
			if (sw_mode() == SW_MODE_AP && nvram_match("re_mode", "0")) {
				...
				nvram_set("wl1_channel", "36"); /* fixed channel speeds up sta1 connection */
				nvram_set("eth_ifnames", " eth0 eth1 eth2 eth3"); /* PLC, LAN1, LAN2 */
			...
			if (nvram_get_int("x_Setting") == 0 || (sw_mode() == SW_MODE_AP && nvram_match("re_mode", "1"))) {
				_dprintf("[%s][%d] sw mode = %d, repeater=%d, ap= %d ",
							__func__, __LINE__,
							sw_mode(),SW_MODE_REPEATER,SW_MODE_AP);
				nvram_set("eth_ifnames", " eth0 eth1 eth4"); /* PLC(eth1), normal WAN(eth4)*/
				nvram_set("fixed_eth_ifnames", "eth4"); /* XP4R WAN(eth4) only as wan */
				nvram_set("amas_ethif_type", "4 4"); /* PLC, 1G */
		...
		nvram_set("amas_lldp_ifnames", " eth0 eth1 eth2 eth3"); /* PLC, LAN1, LAN2 */
		nvram_set("amas_lldp_iftypes", "4 4 4 4"); /* PLC, 1G, 1G */
#endif
		} // XP4
		break;
#endif	/* PLAX56_XP4 */
...
//		nvram_set("bcm43684_nic", "1");	// bring up bcm43684 in nic mode
	...
	case MODEL_RTAX58U_V2:
#ifdef RTCONFIG_COMFW
		nvram_set_int("comfw_type", _PKGTB);
#endif
		nvram_set("lan_ifname", "br0");
		if (is_router_mode()) {
			nvram_set("lan_ifnames", "eth1 eth2 eth3");
